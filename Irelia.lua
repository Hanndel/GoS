...
SpellTable =
	{
	[_Q] = { function() return Dmg = 20+GetCastLevel(myHero, _Q)*30-30 + (GetBonusDmg(myHero) + GetBaseDamage(myHero)) end }
	[_W] = { function() return Dmg = 15*GetCastLevel(myHero, _W) end }
	[_R] = { function() return Dmg = 30+GetCastLevel(myHero, _E)*50 + GetBonusAP(myHero)*0.5 end }
	[_R] = { function() return Dmg = 40+GetCastLevel(myHero, _R)*40 + GetBonusAP(myHero)*0.5 + GetBonusDmg(myHero)*0.6) end, delay = 0.250, speed = 1700, width = 25, range = 1000}
	}
